...
select row_number() over() classificacao, tbr.* from dbcampeonatobr.tb_rodada tbr;


delimiter $$
create procedure dbcampeonatobr.prc_rodadas()
begin 
	declare contador int default 1;
    drop temporary table if exists tb_rodadas;
    create temporary table tb_rodadas(
		rodada int,
        classificacao int,
        sigla varchar(3)
    );
    while contador <= 38 do
		call dbcampeonatobr.prc_rodada(contador);
		insert tb_rodadas 
        select 
			contador as rodada,
			row_number() over() as classificacao, 
            sigla as sigla
		from dbcampeonatobr.tb_rodada tbr;
		set contador = contador + 1;
    end while;
end$$
delimiter ;
-- drop procedure dbcampeonatobr.prc_rodadas;

call dbcampeonatobr.prc_rodadas();
select * from dbcampeonatobr.tb_rodadas;

-- select para saber quantas rodadas o time ficou lider?
select sigla, count(*) qt from dbcampeonatobr.tb_rodadas where classificacao = 1 group by sigla;

-- select para saber quantas rodadas o time ficou na zona de rebaixamento?
select sigla, count(*) qt from dbcampeonatobr.tb_rodadas where classificacao >= 17 group by sigla;